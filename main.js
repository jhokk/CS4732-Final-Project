...
function render() {
    ...
    position[0] += Speed.FAST;
    if (position[0] % 1 === 0) {
        generatePoint(position[0] + controlPoints.length - 3);
    }
...
function generateCatmullRomCurve(segments = 20) {
	...
	let M = mat4( vec4(-0.5, 1.5, -1.5, 0.5), vec4(1, -2.5, 2, -0.5), vec4(-0.5, 0, 0.5, 0), vec4(0, 1, 0, 0) );
...
function generatePoint(x, init=false) {
    if (!init) {
        controlPoints.shift();
    }
